...
\node[anchor=east] (test) at (\Gm@lmargin+\textwidth,-7cm) {\fontsize{180}{100}\sffamily\bfseries\color{ocre!70}\thechapter\strut};
\draw[anchor=west] (\Gm@lmargin,-7cm) node [line width=2pt,rounded corners=0pt,draw=ocre,fill=white,fill opacity=0.8,inner sep=15pt]{\strut\makebox[22cm]{}}; %cadre autour du texte, -9cm, 22cm
...
\draw[anchor=north east] (test.north west) node {\Large\sffamily\bfseries\color{ocre!50} Chapitre }; %modifier ça un peu plus pour que ça marche aussi sur les annexes. essayer de trouver un équivalent de chaptertitlename sans titlesec.
\end{tikzpicture}};
\end{tikzpicture}
\else
\begin{tikzpicture}[remember picture,overlay]
