...
Le transfert se fait dans une pince optique. Il s'agit d'un piège dipolaire focalisé sur les atomes. La forme du piège est alors donnée par la forme du faisceau de piégeage, régie par l'optique gaussienne. Les atomes sont piégés autout du \textit{waist} du faisceau. La longueur d'onde du faisceau de piégeage est $\lambda=$1070nm. La taille transverse du faisceau est $w_{\mathrm{0}}=$28µm. La longueur de piégeage longitudinale est donnée par la longueur de Rayleigh, $z_{\mathrm{R}}= \pi w_{\mathrm{0}}^2 / \lambda$ = 2.3mm. On déplace ce point focal est donc les tomes retenus dedans à l'aide d'une platine de translation sur laquelle sont montées les optiques de focalisation de la pince. Le déplacement de 40cm se fait en 2s. On estime une puissance de 1W sur les atomes. L'efficacité de transfert est estimée à 75%. 

Une fois dans la seconde chambre, on allume un deuxième faisceau afin de comprimer le nuage suivant la direction $\vec{z}$ qui correspond à la direction longitudinale de la pince. Le faisceau \emph{Dimple} est de forme elliptique, de tailles 180µm $\times$ 90µm dans les directions $\vec{x}$ et $\vec{z}$ respectivement. Dans le piège dipolaire croisé, le confinement suivant les directions $\vec{x}$ et $\vec{y}$, est fait par la pince, tandis que le confinement suivant la direction $\vec{z}$ est fait par le faisceau vertical. 7W dans le faisceau vertical. Le faisceau vertical est estimé collimaté.
Le tout est à 10µK, pour un nombre d'atomes estimé à $3 \times 10^6$.
figure transport de vincent, rajouter le dimple dans la seconde chambre.

\paragraph*{Lévitation magnétique}
Afin d'obtenir des temps de propagation dans le désordre très longs et de s'affranchir de toutes les forces extérieures, on doit compenser la gravité. Pour cela, lévitation magnétique. Composée de 2 biais et d'un gradient. 
L'état $\left| F=1, m_{\mathrm{F}}=-1 \right\rangle$ est lévité, les courbures résiduelles sont piégeantes pour le nuage. 
Une autre stratégie est d'utiliser l'état $\left|F=2,m_{\mathrm{F}}=-2 \right\rangle$ qui est \textit{High Field Seeker}, il sera donc expulsé de la lévitation par les courbures résiduelles. 
La gravité diminue la profindeur du piège en le penchant, en s'affranchissant de la gravité on peut aller évaporer beaucoup plus loin: on peut atteindre des températures record.
figure d'illustration de la lévitation.

\paragraph*{Evaporation tout-optique}
Afin de passer le seuil de condensation, on fait une nouvelle étape d'évaporation , cette fois-ci en diminuant la puissance des lasers la profondeur du potentiel de piégeage. Cette étape sera discutée en détail plus tard. Paragraphe à mettre sans doute dans le paragraphe piège optique.

\paragraph*{Refroidir encore plus loin}
On peut vouloir refroidir encore plus.
Ouverture adiabatique du piège: on décomprime la pince en déplaçant la platine de translation de quelques mm: La taille du faisceau au niveau des atomes est plus grande, on diminue les fréquences de piégeage et donc la température.
...
figure delta-kick à créer, basée sur celle de kilian figure 25. 
figure ouverture adiabatique.
