...
\end{equation}
pour un gaz de $N$ particules, avec $N_n$ le nombre moyen d'atomes présents dans l'état d'énergie $E_n$ et de dégénérescence $g_n$, $\mu$ le potentiel chimique, $T$ la température et $k_{\mathrm{B}}$ la constante de Boltzmann. L'origine de cette différence provient de l'indiscernabilité des particules: dans le cadre de la physique classique, les particules identiques sont discernables. Il est possible "d'étiqueter" les particules et de suivre leurs mouvements individuels grâce au déterminisme des lois de la physique classique.
...
\caption{Effet de l'indiscerbabilité des particules}
\label{fig:stat_bose}
\end{figure}
