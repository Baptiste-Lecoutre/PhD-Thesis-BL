...
\subsection{Imagerie}
À la fin de la majorité de nos cycles expérimentaux, nous souhaitons obtenir des informations à propos de notre gaz d'atomes. Une grande partie de ces informations peut être extraite d'une image du nuage, que l'on obtient à l'aide d'une caméra et l'utilisation de lasers à résonance avec les atomes \footnote{L'utilisation de lasers à résonance conduit inévitablement à la destruction du nuage sur notre expérience. Il est donc nécessaire de répéter l'ensemble du cycle expérimental pour obtenir une seconde image des atomes.}.

\paragraph*{Dispositif d'imagerie}
L'expérience est équipée de trois caméras \textit{EMCCD C9102} de chez \textit{Hamamatsu}. Chacune de ces caméras comporte une matrice de $1000 \times 1000$ pixels de taille 8µm $\times$ 8µm. Ces trois caméras sont contrôlées via l'outil d'acquisition d'images de MATLAB, qui permet aussi de récupérer et traiter les images obtenues. L'acquisition des images est déclenchée de manière externe par le séquenceur.\\
Une première caméra acquiert des images des atomes dans la première chambre selon l'axe horizontal $\vec{x}$ avec un grandissement de 1. La zone imageable est donc de 8mm $\times$ 8mm. Il est possible de réaliser de l'imagerie par fluorescence, tout comme de l'imagerie par absorption grâce à un montage 4$f$.\\
Les deux autres caméras sont positionnées autour de la chambre de science selon l'axe horizontal $\vec{x}$ et vertical $\vec{y}$. Toutes deux voient le nuage au travers d'un système optique de grandissement 3\footnote{Ce grandissement a été mesuré on observant la chute libre du nuage en absence de lévitation.}, conduisant à une résolution de 2.71µm pour une zone imageable de 2.71mm $\times$ 2.71mm. Seule de l'imagerie par fluorescence est possible dans cette chambre, en revanche il est possible d'utiliser ces deux caméras simultanément.

\paragraph*{Imagerie par absorption}
Le principe de l'imagerie par absorption repose sur la loi de Beer-Lambert. 
Très basse saturation, $s \ll 1$, en pratique on garde la puissance du faisceau \emph{sonde} en dessous des 100µW pour s'en assurer. Impulsion lumineuse de 50µs avec un laser à résonance avec la transition $\left| F=2 \right\rangle \rightarrow \left| F'=3 \right\rangle$. En pratique, pour mesurer aussi les atomes qui sont dans $\left| F=1 \right\rangle$, on ajoute une impulsion de faisceau de pompage optique pour transférer les atomes dans $\left| F=2 \right\rangle$, impulsion de 40µs accordé sur la transition $\left| F=1 \right\rangle \rightarrow \left| F'=2 \right\rangle$. On fait ce transfert avant le déclenchement de la caméra pour éviter de polluer la mesure. 
\begin{align}
n_{\mathrm{2D}}(y,z) &=\int{\mathrm{d}x \: n(x,y,z)} \\
&= \frac{1}{\sigma} \mathrm{ln}\left( \frac{I_0(y,z)}{I(y,z)} \right)
\end{align}
L'opération de reconstruction du profil de densité atomique nécessite donc 2 images. En pratique, on en prend une troisième afin de soustraire le bruit de fond. Besoin d'un régime où on sature ni la caméra (borne supérieure de l'intensité), ni l'absorption de photons (borne inférieure de l'intensité, il ne faut pas de zéro d'intensité pour des nuages denses).
...
Imagerie par fluorescence dans la première et seconde chambre, que l'on calibre sur l'absorption. En envoie des faisceaux saturants $s \gg 1$ pour que le taux d'émission spontanée ne dépende plus de l'intensité incidente, et on récolte une partie de la lumière diffusée.
\begin{equation}
I_{\mathrm{fluo}}(y,z)=\frac{\Omega}{4\pi} \frac{s}{1+s} \frac{ \Gamma \hb \omega_0}{2} n_{\mathrm{2D}}(y,z)
\end{equation}
avec $\Omega$ l'angle solide dans lequel sont captés les photons de fluorescence, $\Omega \sim \pi \mathrm{ON}^2$.
...
Les faisceaux illuminant les atomes sont à résonance avec la transition $\left| F=2 \right\rangle \rightarrow \left| F'=3 \right\rangle$. Pour aussi sonder les atomes qui sont dans l'état $\left| F=1 \right\rangle$, on y superpose le faisceau repompeur, accordé sur $\left| F=1 \right\rangle \rightarrow \left| F'=2 \right\rangle$ pendant toute la durée de l'impulsion lumineuse. 
L'imagerie dans la première chambre est réalisée à l'aide des faisceaux MOT, tandis que l'imagerie de la seconde chambre est réalisée à l'aide de deux autres faisceaux dédiés. L'impulsion lumineuse dure typiquement 50µs.

\begin{figure}
\centering
\includegraphics[scale=1]{fig/BEC_manip/img_science_3.pdf}
\caption{Dispositif d'imagerie pour la chambre de science. Une caméra capte les photons de fluorescence émis par les atomes selon une direction horizontale (\emph{Science Cam}), tandis qu'une autre capte ceux émis vers le bas avec un transport d'image (\emph{Bottom Cam}).}
