...
Le but de l'étape de refroidissement évaporatif étant d'augmenter la densité dans l'espace des phases $\mathcal{D}=n \ldb^3$ afin de franchir le seuil de condensation, il est donc nécessaire que notre système présente de la dissipation. L'origine de celle-ci repose sur la profondeur finie des pièges utilisés.  
...
Comme mentionné précédemment, la mise en œuvre pratique du refroidissement évaporatif se fait par le biais de la diminution de la profondeur du piège, forçant ainsi le processus d'évaporation. Cette opération est réalisée en continu, et à une vitesse suffisamment lente devant le temps de thermalisation $\tau_{\mathrm{th}}$ afin que le système soit constamment à l'équilibre thermodynamique\footnote{Dans ce cas, les seules pertes d'atomes considérées sont celles liées à l'évaporation par collisions élastiques. Cela revient à négliger la dynamique de l'évaporation, qui fait apparaître les pertes par déversement\citep{cohen1996atomes}. Celles-ci correspondent aux atomes ayant une énergie de $U-\delta E$ qui ne sont plus piégés lors de l'abaissement de la profondeur du piège de $U$ à $U-\mathrm{d}U$, avec $\mathrm{d}U > \delta E$.}. 
...
\begin{equation}
\frac{N(t)}{N(t=0)}=\left( \frac{U(t)}{U(t=0)}\right)^{\frac{3(1-\nu)}{\eta'-3}} \text{ ,}
\end{equation}
avec $\eta'=\eta+\kappa$.


\begin{equation}
\mathcal{D}=N\left(\frac{\hb \overline{\omega}}{\kB T}\right)^3
\end{equation}
