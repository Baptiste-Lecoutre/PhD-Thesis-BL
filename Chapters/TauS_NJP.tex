...
Nous avons pu voir au cours du chapitre \ref{ch:Localisation} l'importance de la détermination précise de la forme de fonctions spectrales. Cette idée n'en a été que renforcée par l'étude détaillée du comportement du temps de diffusion élastique. Dans la perspective d'adresser spécifiquement les niveaux d'énergie du désordre, l'équipe a mis en place un chargement spectroscopique du désordre, permettant la détermination expérimentale des fonctions spectrales pour un désordre de type speckle. Ces mesures ont fait l'objet de la thèse de Vincent Denechaud, où de nombreux détails expérimentaux et théoriques pourront être retrouvés \citep{denechaud2018vers}.

Commençons par réécrire la fonction spectrale:
\begin{align}
A(\mathbf{k}_{\mathrm{i}},E)&=\overline{\left\langle \mathbf{k}_{\mathrm{I}} \left| \delta(E-\hat{H}) \right| \mathbf{k}_{\mathrm{i}} \right\rangle} \\
&= \overline{\sum_{\alpha}{|\langle\mathbf{k}_{\mathrm{i}} | \psi_{\alpha}\rangle|^2.\delta(E-E_{\alpha})}}
\end{align}
en introduisant les états propres $\etat{\psi_{\alpha}}$ du désordre et d'énergie $E_{\alpha}$. Les fluctuations du terme de recouvrement $|\langle \mathbf{k}_{\mathrm{i}} | \psi_{\alpha} \rangle|^2$ disparaissent avec le moyennage sur les différentes réalisations du désordre. On peut alors sortir sa valeur moyenne $\overline{|\langle \mathbf{k}_{\mathrm{i}} | \psi_{\mathrm{E}} \rangle|^2}$ à l'énergie $E$ de la somme:
\begin{align}
A(\mathbf{k}_{\mathrm{i}},E)&\approx \overline{|\langle \mathbf{k}_{\mathrm{i}} | \psi_{\alpha} \rangle|^2}. \overline{\sum_{\alpha}{\delta(E-E_{\alpha})}} \\
&\approx \overline{|\langle \mathbf{k}_{\mathrm{i}} | \psi_{\mathrm{E}} \rangle|^2} . \rho(E) \text{ .}
\end{align}
...
Le protocole de mesure des fonctions spectrales est schématisé sur la figure \ref{fig:illustration_fonction_spectrale}. 
...
réalisation d'un désordre dépendant de l'état interne
utilisation des états horloge pour lesquels le comportement magnétique est identique
transfert RF, résolution de 10Hz limitée par fourier. 
...
\begin{figure}
\centering
\includegraphics[width=\textwidth]{Fig/TauS_NJP/aek0_panel_red.pdf}
\includegraphics[width=\textwidth]{Fig/TauS_NJP/aek0_panel_blue.pdf}
\caption{\textbf{Mesure des fonctions spectrales.} Stuff.}
\label{fig:mesures_fonctions_spectrales}
\end{figure}


\subsection{Comparaison du temps de diffusion élastique avec les fonctions spectrales mesurées}
extraction du temps de diffusion élastique des fonctions spectrales mesurées.

comparaison pour le rouge et pour le bleu .
