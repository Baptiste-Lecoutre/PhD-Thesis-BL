...
\end{equation}
dans la base des énergies \citep{akkermans2007mesoscopic, kuhn2007coherent}. En remplaçant le hamiltonien $\hat{H}$ par son expression, on montre ainsi que la fonction de Green est solution de l'équation de Dyson
\begin{equation}
...
\label{eq:green_libre}
\end{equation}
avec $E_{\mathrm{k}_i}$ l'énergie cinétique de la particule. 

À l'aide de l'équation de Dyson \ref{eq:dyson} et en identifiant les processus irréductibles\footnote{Un processus irréductible correspond à un processus de diffusion qui ne peut pas se \emph{factoriser} comme une succession d'évènements élémentaires indépendants de diffusion. Cette notion prend tout son sens dans le cadre de l'approche diagrammatique à la détermination de la fonction de Green, au-delà de la portée de cette thèse.}, on peut montrer la fonction de Green moyenne se met sous la forme
\begin{equation}
...
\end{equation}
où $\overline{\cdots}$ représente la moyenne sur les différentes réalisations du désordre \citep{akkermans2007mesoscopic, kuhn2007coherent}. La quantité complexe $\Sigma(E,\mathbf{k}_{\mathrm{i}})$, la \emph{Self-Energy}, est homogène à une énergie et encode l'effet des fluctuations du potentiel désordonné $\hat{V}$\footnote{Seules les fluctuations du potentiel sont contenues dans la \selfenergy\ telle qu'écrite ici puisque nous avons fait apparaître explicitement la valeur moyenne du potentiel.}. Si l'expression de la fonction de Green moyenne \ref{eq:fonction_green_moyenne} est exacte, sa détermination reste ardue et demande l'utilisation d'approximations pour déterminer la \selfenergy , telles que l'expansion de Born \citep{kuhn2007coherent, lugan2009one}, l'approche auto-consistante de Born \citep{skipetrov2008anderson, yedjour2010diffusion}, ou encore des approximations semi-classiques \citep{prat2016semiclassical, trappe2015semiclassical}. 
...
\begin{equation}
A(\mathbf{k}_{\mathrm{i}},E)=-\frac{1}{\pi} \overline{\langle \mathbf{k}_{\mathrm{i}} | \mathrm{Im}\left[\hat{G}(E)\right]| \mathbf{k}_{\mathrm{i}} \rangle} = \overline{\langle \mathbf{k}_{\mathrm{i}} | \delta(E-\hat{H})|\mathbf{k}_{\mathrm{i}}\rangle} \text{ ,}
\end{equation}
...
\begin{equation}
\overline{U}_{\mathrm{k}_{\mathrm{i}}}(t)=\left\langle \mathbf{k}_{\mathrm{i}} \left| \overline{\exp(-it\hat{H}/\hb)}  \right| \mathbf{k}_{\mathrm{i}} \right\rangle = \int{\diff E e^{-iEt/\hb} A(\mathbf{k}_{\mathrm{i}},E)} \text{ .}
\end{equation}
...
Afin d'estimer quantitativement la \selfenergy , une méthode couramment utilisée consiste à décomposer cette dernière sous la forme d'une série
%En itérant l'équation de Dyson \ref{eq:dyson} et en moyennant sur les différentes réalisations du désordre, on peut décomposer la \selfenergy\ sous la forme d'une série
%\begin{equation}
...
%\end{equation}
\begin{equation}
\hat{\Sigma}(E) = \hat{\Sigma}^{(1)}(E) + \hat{\Sigma}^{(2)}(E) + \cdots \text{ ,}
\label{eq:developpement_born}
\end{equation}
connue sous le nom de développement de Born. La \selfenergy\ est alors donnée par une somme infinie de termes $\hat{\Sigma}^{(i)}$, où chaque terme est composé de $i+1$ occurrences du potentiel $\hat{V}$ \citep{kuhn2007coherent}. Notamment, les premiers termes du développement de Born s'expriment $\hat{\Sigma}^{(1)}(E)=\overline{\hat{V} \hat{G}_0(E) \hat{V}}$ et $\hat{\Sigma}^{(2)}(E)=\overline{\hat{V} \hat{G}_0(E) \hat{V} \hat{G}_0(E) \hat{V}}$.

Dans la limite de désordre faible, il apparaît de l'équation \ref{eq:developpement_born} que seul le premier terme $\hat{\Sigma}^{(1)}$ contribue à la \selfenergy , chaque occurrence supplémentaire du potentiel atténuant le poids du terme $\hat{\Sigma}^{(i)}$ correspondant. L'approximation de Born consiste donc à tronquer l'expression de la \selfenergy\ à son premier terme:
\begin{equation}
\Sigma(E,\mathbf{k}_{\mathrm{i}})\approx \Sigma^{(1)}(E,\mathbf{k}_{\mathrm{i}})= \left\langle \mathbf{k}_{\mathrm{i}} \left|\overline{\hat{V}\hat{G}_0(E) \hat{V}}\right| \mathbf{k}_{\mathrm{i}}\right\rangle \text{ .}
\end{equation}

On montre dans l'annexe \ref{ch:anex_taus} que le premier terme du développement de Born $\Sigma^{(1)}(E,\mathbf{k}_{\mathrm{i}})$  peut se réécrire comme la convolution du spectre des fréquences spatiales du désordre avec la fonction de Green libre \ref{eq:green_libre}:
\begin{equation}
...
On peut montrer que le premier terme du développement $\Sigma^{(1)}(E,\mathbf{k}_{\mathrm{i}})$ varie peu autour de l'énergie cinétique, ce qui permet de remplacer la \selfenergy\ par sa valeur en couche $E=\Eki$ \citep{kuhn2007coherent}. La fonction spectrale obtenue à l'aide de l'équation \ref{eq:fonction_spectrale_self_energy} présente alors un profil lorentzien:
\begin{equation}
A(\mathbf{k}_{\mathrm{i}},E)= \frac{1}{\pi} \frac{\Delta E/2}{(E - \Eki')^2+\Delta E^2/4} \text{ ,}
\end{equation}
...
Comme nous le verrons plus en détails dans la section \ref{sc:comparaison_fonctions_spectrales} et sur les figures \ref{fig:comparaison_taus_specfunc_rouge} et \ref{fig:comparaison_taus_specfunc_bleu}, la SCBA fournit une meilleure prédiction du temps $\taus^{\mathrm{scba}}$ que l'approximation de Born au premier ordre. Néanmoins, celle prédiction reste identique pour les \speckles\ attractif et répulsif, en désaccord avec le comportement de nos données expérimentales, particulièrement dans le régime classique où de fortes déviations entre désordres attractif et répulsif apparaissent (voir la discussion des fonctions spectrales dans la section \ref{sc:mesure_fonction_spectrale}). Cet effet montre clairement le manquement de la \textit{SCBA} à décrire le régime classique pour lequel la distribution de potentiel est capitale. Il apparaît donc nécessaire d'analyser les détails de la fonction spectrale pour un désordre de type \speckle .
...
De nombreuses hypothèses sont nécessaires pour obtenir ce résultat, dont une discussion détaillée est donnée dans les références \citep{denechaud2018vers, volchkov2018measurement}. Notamment, cette correspondance entre fonction spectrale et taux de transfert n'est valide que pour un couplage faible ($\Omega$ petit devant les autres échelles d'énergie) et des temps $t$ courts tels que $\Gamma t \ll 1$. Une conséquence forte de ces contraintes est que seulement quelques pourcents des atomes sont transférés dans le désordre. 
...
\label{sc:comparaison_fonctions_spectrales}
...
\caption{\textbf{Comparaison des fonctions spectrales mesurées au temps de diffusion élastique pour un \speckle\ attractif.} \textbf{a:} Le temps de diffusion élastique est comparé à la prédiction de l'approximation auto-consistante de Born. Cette dernière est proche des données expérimentales dans le régime de désordre faible, mais ne permet pas de décrire le régime de désordre fort. La SCBA échoue de plus à décrire les asymétries du potentiel. Néanmoins, cette dernière fournit une meilleure prédiction que celle issue de l'approximation de Born au premier ordre. Les temps de vie $\taus^{\mathrm{sf}}$ extraits des fonctions spectrales mesurées pour $\etat{\mathbf{k}=0}$ sont compatibles avec les mesures de $\taus$ de basse impulsion ($k_{\mathrm{i}}=0.33\sigmap^{-1}$). \textbf{b:} Illustration de l'extraction du temps de vie $\taus^{\mathrm{sf}}$. Le temps de vie est extrait en déterminant la largeur à mi-hauteur d'une fonction d'ajustement des données expérimentales. Ces données sont celles de la figure \ref{fig:mesures_fonctions_spectrales_rouge}.}
...
\section{Conclusion}
Dans ce chapitre, nous nous sommes focalisés sur la description du temps de diffusion élastique dans le régime de désordre fort à l'aide du concept général des fonctions spectrales. En particulier, nous avons démontré la cohérence de nos deux approches:
\begin{itemize}
\item[\textendash] L'approche temporelle qui consiste à suivre l'évolution temporelle du l'état initial afin de déterminer un temps caractéristique $\taus$.
\item[\textendash] L'approche spectrale décrivant la distribution d'énergie de l'état initial plongé dans le désordre, de laquelle il est possible d'extraire un temps de vie caractéristique $\taus^{\mathrm{sf}}$.
\end{itemize}


Cette étude heuristique, reposant sur l'identification d'un temps caractéristique, illustre donc notre très bonne connaissance des propriétés de diffusion élémentaire en vertu de l'accord entre nos données expérimentales, basées sur des protocoles de mesures indépendants. Cette étude constitue alors un premier vers l'étude des propriétés de transport.