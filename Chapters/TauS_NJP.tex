...
%\end{equation}
\begin{equation}
\Sigma = \Sigma^{(1)} + \Sigma^{(2)} + \cdots \text{ ,}
\label{eq:developpement_born}
\end{equation}
connue sous le nom de développement de Born. La \selfenergy\ est alors donnée par une somme infinie de termes $\Sigma^{(i)}$, où chaque terme est composé de $i+1$ occurrences du potentiel $\hat{V}$ \citep{kuhn2007coherent}. Notamment, les premiers termes du développement de Born s'expriment $\Sigma^{(1)}=\overline{V G_0 V}$ et $\Sigma^{(2)}=\overline{V G_0 V G_0 V}$.
