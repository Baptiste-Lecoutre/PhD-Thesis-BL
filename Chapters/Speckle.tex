...
\end{equation}
où l'intensité $I_0'(\xzero)$ dans le plan du diffuseur vu sous l'ouverture numérique effective est donnée par
\begin{equation}
I_0'(\xzero)= \exp{\left( -\frac{2 \xzero^2}{w_{\mathrm{eff}}^2} \right)} \: \Theta \left( \left| \xzero \right|- \frac{D_{\mathrm{eff}}}{2} \right)
\end{equation}
avec $D_{\mathrm{eff}}=\xscale D$ et $w_{\mathrm{eff}}=\xscale w_0$ les tailles effectives du faisceau.
...
étude de la différence des potentiels de même amplitude pour voir à quel point ils s'annulent sur F=1. cette étude de la différence des potentiels revient à regarder la différence des intensités de mêmes valeurs moyennes. 
\begin{equation}
...
Etude de la fonction de corrélation entre les deux amplitudes au même point à deux longueurs d'ondes différentes.
De cette manière, on fait apparaître la fonction de corrélation du diffuseur
\begin{equation}
\Cdiff(\xzero,\xzero',\lambda_{\mathrm{p}},\lambda_{\mathrm{c}})=\overline{\tdiff(\xzero,\lambda_{\mathrm{p}})\tdiff^*(\xzero',\lambda_{\mathrm{c}})}
\end{equation}
On montre qu'elle vaut
\begin{equation}
...
Aux alentours du plan de Fourier, on montre que la fonction de corrélation à 3D est donnée par
\begin{equation}
c_{\mathrm{2}\lambda} (\mathbf{x},\delta z) =\exp{\left( -\delta\phi^2 \right)}\times c_{\mathrm{3D}}\left( \mathbf{x} \frac{\delta\lambda}{\lambda}, \delta z\frac{\delta\lambda}{\lambda} \right)
\end{equation}

Signification des deux termes: le scaling en $\delta\lambda/\lambda$ correspond à la cohérence temporelle. Analogie avec le doublet du sodium sur un michelson: tailles de grains différentes, il faut aller chercher $\lambda/\delta\lambda$ grains pour qu'ils ne soient plus superposés.
Le premier terme correspond à une décorrélation par la traversée du diffuseur. $\tdiff(\xzero,\lambda)$. Le déphasage accumulé localement par l'onde n'est pas exactement le même.
