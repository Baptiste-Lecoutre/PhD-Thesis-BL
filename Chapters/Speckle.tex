...
à l'echelle des grains de speckle, on peut supposer que l'intensité lumineuse moyenne est constante. Dans ce cas, on montre à l'aide du théorème de Wick pour les variables gaussiennes que la fonction de corrélation des fluctuations d'intensité s'écrit
\begin{equation}
\overline{\delta I(\xd) \delta I(\xd')} \propto \left| \overline{E(\xd)E^*(\xd')} \right|^2
\end{equation}
avec $\delta I(\xd)=I(\xd)-\overline{I}(\xd)$.

On montre annexe \ref{ch:anex_speckle} que la fonction transverse le long de l'axe optique peut s'écrire 
\begin{align}
\nonumber \CE(\xd,\xd') &= \overline{E(\xd) E^*(\xd')} \\
& \propto \widetilde{I_0}\left( \frac{\delta\xd}{\lambda d}\right) \ast \Cdiff\left( \frac{\deff}{d} \delta \xd \right)
\end{align}
où $\delta\xd=\xd'-\xd$. Deux contributions.
\begin{itemize}
\item[•] le premier terme correspond à la TF de l'intensité lumineuse dans le plan du diffuseur, avec un scaling proportionnel à la distance. Ce scaling est dû à la diminution de l'ouverture numérique au fur et à mesure de la distance, $\ON=D/2d$. Donc au fur et à mesure de la propagation, l'ouverture numérique diminue et ça conduit à un élargissement de la tâche de diffraction en absence de diffuseur.
\item[•] le second terme tient compte de la diffraction par les grains selon un angle $\theta_{\mathrm{diff}}$. Il joue le rôle d'ouverture numérique effective, et indique que loin du plan de Fourier, tous les grains ne participent pas à l'éclairement en un point donné. Mettre le schéma pour expliquer tout ça, sinon c'est imbitable.
\end{itemize}

Tout comme pour l'extension du champ de speckle, deux régions dont les frontières sont les mêmes.
Une région loin du plan de Fourier où l'ouverture numérique effective faible, conduit à des gros grains de speckle. Autour du plan de Fourier, le terme de TF de l'intensité au niveau du diffuseur est prépondérant. Dans ce régime,
On montre alors que la fonction de corrélation aux alentours du plan de Fourier peut s'écrire 
\begin{equation}
\overline{\delta I (\xf) \delta I (\xf + \delta \xf)} \propto \left| \mathrm{TF} \left[ I_0 \right] (\delta \xf / \lambda f \right| ^2
\end{equation}
La tailles des grains est alors donnée par la limite de diffraction du système, $\sigmap\sim \lambda/2\ON$
