...
phrases d'intro pour expliquer les comparaisons aux modèles et au critère de désordre faible.
...
La région de fort désordre sort naturellement du cadre de la théorie perturbative de Born. Il s'agit de la région pour laquelle la distribution de potentiel joue un rôle capital, comme cela est illustré par la différence significative des déviations que l'on peut observer dans cette région pour les différents désordres. Nous verrons dans le chapitre suivant que dans ce régime, l'évolution temporelle de l'état initial est décrite par sa distribution d'énergie, donnée par la fonction spectrale $A(\mathbf{k}_{\mathrm{i}},E)$ et dont le profil est intimement relié à la distribution de potentiel.


\subsection{Départ quadratique}
parler du début de la décroissance qui n'est pas linéaire comme pour une exponentielle mais quadratique. Ce départ est bien vu dans les numériques même dans le régime de Born, mais pas sur la manip car durée associée trop courte.
interprétation à l'aide du bouquin d'alain: à temps court, le couplage de la règle d'or de fermi ne peux pas satisfaire la condition de conservation de l'énergie et la fonction de couplage en énergie n'a pas encore convergé vers un dirac. on sonde ainsi un continuum de largeur finie $\ER$ (impulsion cutoff du désordre, transfert d'énergie cinétique), qui s'apparente à temps court à une oscillation de rabi (vu par la fonction de spectroscopie large en énergie, le continuum s'apparente à un niveau discret).
On a donc deux régimes: un régime non résolu qui commence comme une oscillation de Rabi, puis la résolution en énergie s'améliore, on sonde les différents niveaux et on retrouve le comportement exponentiel.


\citep{grynberg2010introduction}

\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{Fig/TauS_PRL/depart_quadratique_taus.pdf}
\caption{\textbf{Départ quadratique de la décroissance de l'état initial.} Stuff.}
